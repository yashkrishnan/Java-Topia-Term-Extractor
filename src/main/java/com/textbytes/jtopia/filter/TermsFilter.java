...
public class TermsFilter {
    ...
    private DateUtils dtOffset = new DateUtils();
    ...
    public Map<String, ArrayList<Integer>> filterTerms(Map<String, Integer> extractedTerms) {
        ...
        Map<String, ArrayList<Integer>> finalFilteredTerms;
     ...
     * @return
     */
    private Map<String, ArrayList<Integer>> removeDuplicateSingleWords(Map<String, ArrayList<Integer>> filteredTerms) {
        Set<String> keySet = filteredTerms.keySet();
     ...
     * @return
     */
    private boolean isWordPresent(String singleTerm, Set<String> keySet) {
        singleTerm = singleTerm.trim().toLowerCase();
        for (String termInMap : keySet) {
            ...
            if (termArray != null && termArray.length > 1) {
                for (String aTermArray : termArray) {
                    if (aTermArray.trim().toLowerCase().equals(singleTerm)) {
            ...
            for (Object key : keySet) {
                ...
                for (String word : stopWords) {
                    if (termArray != null) {
                        for (String aTermArray : termArray) {
                            ...
                            if (aTermArray.trim().toLowerCase().equals(word.trim())) {
                                isStopWrod = true;
                                break;
                            } else if (hasDigitOrPunctuation(aTermArray.trim())) {
                ...
                if (!isStopWrod && !hasDigitOnlyWord) {
                    ...
                    if (term.length() > 3 && !isDigitOnly(term)) {
                        ...
                        logger.info("Discarding term " + term + " with length " + term.length());
                    }
                } else {
                    logger.info("Discarding term " + term + " isStopword "
     ...
     * @return
     */
    private Map<String, ArrayList<Integer>> removeDateTerms(Map<String, ArrayList<Integer>> filterdTerms) {
